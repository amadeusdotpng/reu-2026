...
In this system, there are 12 tile types evenly split into four categories:
\verb|Border|, \verb|Zero|, \verb|One|, and \verb|Two| as shown in \cref{fig:exs-1}.
...
The seed assembly contains a singular \verb|SEED| tile with which the \verb|BOTTOM| and \verb|RIGHT|
...
In this system, there are 12 tiles evenly split between two categories: \verb|Zero| and \verb|One|
as shown in \cref{fig:exs-2}.
...
This counter works similarly to the previous counter by thinking of its south and east sides as
inputs, its north and west sides as its outputs, and the tile itself as the result of the operation
on the inputs.
talk about:

also passes values along the glue.

passes along another type of ``information'' \verb|G| indicating whether or not it should eventually
generate another column on the left.

Concept of ``epsilon'' input.
